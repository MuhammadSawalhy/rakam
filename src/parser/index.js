...
class Parser {
  ...
  __parsed2mathNode_math(parserTree) {
    ...
    } else if (expr.type === "variable") return new Variable(expr.name);
    else if (expr.args.length == 2) {
      {
        if ((expr.calls("&&", 2) && expr.args[0] && expr.args[1]) || (expr.calls("and", 2) && expr.args[0] && expr.args[1]))
          return new And(snodeTOnode(expr.args[1]), snodeTOnode(expr.args[2]));
        else if ((expr.calls("||", 2) && (expr.args[0] = expr.args[0]) && expr.args[1]) || (expr.calls("or", 2) && (expr.args[0] = expr.args[0]) && expr.args[1])) {
          let a = snodeTOnode(expr.args[0]),
            b = snodeTOnode(expr.args[1]);
          ...
          expr.calls("constrain", 2) &&
          expr.args[0] &&
          expr.args[1] &&
          (expr.args[1].calls("()", 2) || expr.args[1].calls("[]", 2)) &&
          expr.args[1].args[0] &&
          expr.args[1].args[1]
        )
...

    {
      ...
      else if (expr.calls("?", 2, "op") && expr.args[0] && expr.args[1]) return new If([snodeTOnode(expr.args[0]), snodeTOnode(expr.args[1]), null]);
      else if (expr.calls("if", 2, "func") && expr.args[0] && expr.args[1]) return new If([snodeTOnode(expr.args[0]), snodeTOnode(expr.args[1]), null]);
      else if (expr.calls("if", 3, "func") && expr.args[0] && expr.args[1] && expr.args[2]) return new If([snodeTOnode(a), snodeTOnode(expr.args[1]), snodeTOnode(expr.args[2])]);
      ...
      else if (expr.calls("random", "func")) return _getRandom(expr);
      else if (expr.calls("min", "func")) {
        let childres = new Array(expr.args.length);
        for (let i = 0; i < expr.args.length; i++) {
          if (expr.args[i] == null) throw new Exception("Passed argument musn't be void.");
